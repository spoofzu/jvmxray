...
import org.jvmxray.util.DBUtil;
 ...
 */
@WebServlet(
		name = "JVMXRayListener",
		description = "Java IASP monitor",
		loadOnStartup = 1,
		urlPatterns = "/api/*"
)
...
//				}
	...
	private void addEvent(String event) throws IOException, SQLException {
		dbutil.insertEvent(dbconn,
				event );
	}

	private final void initDB() throws SQLException, IOException {
		PropertyUtil pu = PropertyUtil.getInstance(PropertyUtil.SYS_PROP_SERVER_DEFAULT);
		Properties p = pu.getProperties();
    ...
    private void echoPage(HttpServletResponse res, String content) throws Exception {
	    ...
	    if (content != null && content.length() > 0 ) {
		    event = content;
		    et = ct - EventUtil.getInstance().getTimeStamp(event);
	    } else {
