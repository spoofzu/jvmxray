...
public class MonitorSensor extends AbstractSensor implements Sensor {
    ...
    private void logSystemStats() {
        ...
        // Collect LogProxy buffer metrics
        try {
            AgentLogger.LogProxyMetrics logMetrics = AgentLogger.getInstance().getMetrics();
            stats.put("LogBufferUtilization", logMetrics.getBufferUtilization());
            stats.put("LogQueueSize", logMetrics.getCurrentQueueSize());
            stats.put("LogDiscardCount", logMetrics.getDiscardCount());
            stats.put("LogFlushRate", logMetrics.getFlushRate());
            stats.put("LogFlushTime", logMetrics.getAvgFlushTime());
            stats.put("LogOverflowStrategy", logMetrics.getOverflowStrategy());
