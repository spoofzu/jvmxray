...
import org.jvmxray.agent.util.AgentLogPair;
...
public class jvmxraysecuritymanager extends SecurityManager {
    ...
    public void checkSecurityAccess(String target) {
        ...
        try (MDC.MDCCloseable mdcCloseable = MDC.putCloseable(VERSION_TAG, VERSION_ID)) {
            ...
            String P3       = "";
            logger.warn("{} {} {} {} {} {} {}",
                    AgentLogPair.value("EVENTTP", EVENTTP),
                    AgentLogPair.value("EVENTID", eventid),
                    AgentLogPair.value("AID", aid),
                    AgentLogPair.value("CATEGORY", cat),
                    AgentLogPair.value("P1", P1),
                    AgentLogPair.value("P2", P2),
                    AgentLogPair.value("P3", P3)
            );
        ...
        IntStream.range(0, eventMetaArray.length).forEach(i -> {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("STACKFRM {} {} {} {} {} {} {} {} {} {} {} {} {} {}",
                        AgentLogPair.value("LVL", Integer.toString(i)),
                        AgentLogPair.value("EVENTTP", EVENTTP),
                        AgentLogPair.value("EVENTID", eventId),
                        AgentLogPair.value("AID", aid),
                        AgentLogPair.value("CATEGORY", category),
                        AgentLogPair.value("CLZ_LDR", eventMetaArray[i].getClsLoaderNm()),
                        AgentLogPair.value("CLZ_CN", eventMetaArray[i].getClsNm()),
                        AgentLogPair.value("CLZ_METHNM", eventMetaArray[i].getMethNm()),
                        AgentLogPair.value("CLZ_MODNM", eventMetaArray[i].getModuleNm()),
                        AgentLogPair.value("CLZ_MODVR", eventMetaArray[i].getModuleVr()),
                        AgentLogPair.value("CLZ_FILENM", eventMetaArray[i].getFileNm()),
                        AgentLogPair.value("CLZ_LINENO", Integer.toString(eventMetaArray[i].getLineNum())),
                        AgentLogPair.value("CLZ_LOCATION", eventMetaArray[i].getLoc()),
                        AgentLogPair.value("CLZ_NATIVE", Boolean.toString(eventMetaArray[i].isIsNative()))
                );
